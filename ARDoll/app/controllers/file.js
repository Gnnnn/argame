...
function fileProcess(file, callback) {
...
function responseFile(fileId, req, res, callback) {
	var gfs = require('../app').gfs,
		options = {_id: fileId};
...
module.exports = {
	fileUpload: function (req, callback) {
		return async.map(req.files,
			function (file, aCallback) {
				fileProcess(file, aCallback);
			},
			callback);
	},
	getFile   : function (req, res, next) {
		serverUtils.setCacheControl(res, 'max');
		responseFile(req.params.id, req, res, next);
	},
	removeFile: function (req, callback) {
		var gfs = require('../app').gfs;
