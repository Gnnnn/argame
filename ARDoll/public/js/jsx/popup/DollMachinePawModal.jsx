...
import {updateDollMachinePaw} from '../../actions/api';
import {closeDollMachinePawModal, createDollMachinePaw} from '../../actions/client';
...
const DollMachinePawModal = React.createClass({
	displayName: 'DollMachinePawModal',
	...
	afterOpenFn() {
		let {openDollMachinePawId, dollMachinePaws} = this.props;
		let dollMachinePaw;
		if (openDollMachinePawId && dollMachinePaws) {
			dollMachinePaw = dollMachinePaws.get(openDollMachinePawId);
		}
		if (dollMachinePaw) {
			this.setState({
				name                 : dollMachinePaw.get('name'),
				description          : dollMachinePaw.get('description'),
				thumbnail            : dollMachinePaw.get('thumbnail'),
				showImageUpload      : false,
				imageId              : '',
				document             : dollMachinePaw.get('document'),
				fileId               : '',
				showFileUpload       : false,
				androidDocument      : dollMachinePaw.get('androidDocument'),
				androidFileId        : '',
				showAndroidFileUpload: false
			});
		}
	...
	handleSave() {
		let name = this.refs.name.value,
			...
			{openDollMachinePawId} = this.props;
		let data = {name, description, thumbnail, document, androidDocument};
		if (openDollMachinePawId) {
			this.props.updateDollMachinePaw(openDollMachinePawId, data);
		}
		else {
			this.props.createDollMachinePaw(data);
		}
	},
	handleClose() {
		this.props.closeDollMachinePawModal();
	},
	...
	render() {
		...
		return (
			<Modal isOpen = {isActive} onAfterOpen = {this.afterOpenFn} modalName = "DollMachinePawModal"
			       style = {customStyles}>
...
function mapStateToProps(state) {
	return {
		isActive            : state.getIn(['app', 'appInfo', 'modals', constants.MODAL_DOLL_MACHINE_PAW]),
		users               : state.getIn(['data', 'users']),
		dollMachinePaws     : state.getIn(['data', 'dollMachinePaws']),
		openDollMachinePawId: state.getIn(['app', 'appInfo', 'openDollMachinePawId']),
		onSending           : state.getIn(['app', 'appInfo', 'onSending'])
	};
}

export default connect(mapStateToProps, {
	updateDollMachinePaw,
	closeDollMachinePawModal,
	createDollMachinePaw
})(DollMachinePawModal);