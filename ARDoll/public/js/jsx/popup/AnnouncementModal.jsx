...
import {updateAnnouncement} from '../../actions/api';
import {closeAnnouncementModal, createAnnouncement} from '../../actions/client';
...
const AnnouncementModal = React.createClass({
	displayName: 'AnnouncementModal',
	...
	afterOpenFn() {
		let {openAnnouncementId, announcements} = this.props;
		let announcement;
		if (openAnnouncementId && announcements) {
			announcement = announcements.get(openAnnouncementId);
		}
		if (announcement) {
			this.setState({
				name           : announcement.get('name'),
				tag            : announcement.get('tag'),
				thumbnail      : announcement.get('thumbnail'),
				showImageUpload: false,
				imageId        : '',
				document       : announcement.get('document'),
				fileId         : '',
				showFileUpload : false
			});
		}
	...
	handleSave() {
		let name = this.refs.name.value,
			...
			{openAnnouncementId} = this.props;
		let data = {name, tag, thumbnail, document};
		if (openAnnouncementId) {
			this.props.updateAnnouncement(openAnnouncementId, data);
		}
		else {
			this.props.createAnnouncement(data);
		}
	},
	handleClose() {
		this.props.closeAnnouncementModal();
	},
	...
	render() {
		let {isActive} = this.props,
			{name, tag} = this.state;
		return (
			<Modal isOpen = {isActive} onAfterOpen = {this.afterOpenFn} modalName = "AnnouncementModal"
			       style = {customStyles}>
				...
				<div className = "row">
					...
					<div className = "large-4 columns">
						<label>标签：
							<input id = "tag" type = "text" ref = "tag"
							       value = {tag}
...
function mapStateToProps(state) {
	return {
		isActive          : state.getIn(['app', 'appInfo', 'modals', constants.MODAL_ANNOUNCEMENT]),
		users             : state.getIn(['data', 'users']),
		announcements     : state.getIn(['data', 'announcements']),
		openAnnouncementId: state.getIn(['app', 'appInfo', 'openAnnouncementId']),
