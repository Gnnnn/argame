...
import {updateDollMachine} from '../../actions/api';
import {closeDollMachineModal, createDollMachine} from '../../actions/client';
...
const DollMachineModal = React.createClass({
	displayName: 'DollMachineModal',
	...
	afterOpenFn() {
		let {openDollMachineId, dollMachines} = this.props;
		let dollMachine;
		if (openDollMachineId && dollMachines) {
			dollMachine = dollMachines.get(openDollMachineId);
		}
		if (dollMachine) {
			this.setState({
				name                 : dollMachine.get('name'),
				description          : dollMachine.get('description'),
				thumbnail            : dollMachine.get('thumbnail'),
				showImageUpload      : false,
				imageId              : '',
				document             : dollMachine.get('document'),
				fileId               : '',
				showFileUpload       : false,
				androidDocument      : dollMachine.get('androidDocument'),
				androidFileId        : '',
				showAndroidFileUpload: false
			});
		}
	...
	handleSave() {
		let name = this.refs.name.value,
			...
			{openDollMachineId} = this.props;
		let data = {name, description, thumbnail, document, androidDocument};
		if (openDollMachineId) {
			this.props.updateDollMachine(openDollMachineId, data);
		}
		else {
			this.props.createDollMachine(data);
		}
	},
	handleClose() {
		this.props.closeDollMachineModal();
	},
	...
	render() {
		...
		return (
			<Modal isOpen = {isActive} onAfterOpen = {this.afterOpenFn} modalName = "DollMachineModal"
			       style = {customStyles}>
...
function mapStateToProps(state) {
	return {
		isActive         : state.getIn(['app', 'appInfo', 'modals', constants.MODAL_DOLL_MACHINE]),
		users            : state.getIn(['data', 'users']),
		dollMachines     : state.getIn(['data', 'dollMachines']),
		openDollMachineId: state.getIn(['app', 'appInfo', 'openDollMachineId']),
