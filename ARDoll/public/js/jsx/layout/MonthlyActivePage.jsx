...
import {loadMonthlyActive} from '../../actions/api';
...
const RetentionPage = React.createClass({
	handleExport() {
		let url = tsResourzes.Active.getComputedUrl({type: 'monthly'}) + '/excel' + '#' + Math.random();
		this.props.exportExcel(url);
	},
	componentWillReceiveProps(nextProps) {
		if (nextProps.loginUserId && this.props.loginUserId !== nextProps.loginUserId) {
			this.props.loadMonthlyActive();
		}
		if (nextProps.monthlyActives !== this.props.monthlyActives) {
			let date = getDateArray(nextProps.monthlyActives),
				data = getCount(nextProps.monthlyActives);
...
function mapStateToProps(state) {
	return {
		...
		monthlyActives: state.getIn(['app', 'actives', 'monthly'])
	};
}
